...
A register-width grid, `text-align: left`, hover `bg-raised` in 140ms. The open Playlist row holds `bg-raised`; its title goes 600 `mood-ink`; unselected stays 500 `ink`. Current track title is 500 `ink`; others `ink-2`. Current Position is `mood-ink`. Figures right-align. No chevron, no thumbnail, no card chrome.
